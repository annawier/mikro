...
sd_glm <- sqldf("
                ...
                home_team_api_id opponent_team_api_id
                from pozycja_2AWAY
                ")
...
nrow(sd_glm)
nrow(sd_glm_merged) ##usun1a duplikaty z sd_glm_merged
...
sd_fifa_dates<-unique(sd_fifa_dates)

head(sd_fifa_dates)
colnames(sd_fifa_dates)
...
#Wywalenie drugiej zmiennej o nazwie #id dates team_api_Id
sd_glm_merged<-sd_glm_merged[,c(-56,-58,-59)]
...
team_attr_list <- c("buildUpPlaySpeedClass",
...
for(i in team_attr_list) {
  sd_glm_merged_3[[paste('O', i, sep="_")]] <- paste(sd_glm_merged_3[[i]],sd_glm_merged_3[[paste('O', i, sep="_")]])
}
